...
public class PushMessageReceiver extends BroadcastReceiver {
	...
	public void onReceive(final Context context, Intent intent) {
		...
		if (intent.getAction().equals(PushConstants.ACTION_MESSAGE)) {
			...
			// // 娑堟伅鐨勭敤鎴疯嚜瀹氫箟鍐呭璇诲彇鏂瑰紡
			// Log.i(TAG, "onMessage: " + message);
			//
			// // 鐢ㄦ埛鍦ㄦ鑷畾涔夊鐞嗘秷鎭��浠ヤ笅浠ｇ爜涓篸emo鐣岄潰灞曠ず鐢��
			...
			// 澶勭悊缁戝畾绛夋柟娉曠殑杩斿洖鏁版嵁
			// PushManager.startWork()鐨勮繑鍥烇拷1锟�斤拷1锟�借繃PushConstants.METHOD_BIND寰楀埌

			// 鑾峰彇鏂规硶
			final String method = intent
					.getStringExtra(PushConstants.EXTRA_METHOD);
			// 鏂规硶杩斿洖閿欒鐮侊拷1锟�借嫢缁戝畾杩斿洖閿欒锛堥潪0锛夛紝鍒欏簲鐢ㄥ皢涓嶈兘姝ｅ父鎺ユ敹娑堟伅銆��
			// 缁戝畾澶辫触鐨勫師鍥犳湁澶氱锛屽缃戠粶鍘熷洜锛屾垨access token杩囨湡銆��
			// 璇蜂笉瑕佸湪鍑洪敊鏃惰繘琛岀畝鍗曠殑startWork璋冪敤锛岃繖鏈夊彲鑳藉鑷存寰幆銆��
			// 鍙互閫氳繃闄愬埗閲嶈瘯娆℃暟锛屾垨鑰呭湪鍏朵粬鏃舵満閲嶆柊璋冪敤鏉ヨВ鍐筹拷1锟���
			...
			// 鍙拷1锟�斤拷1锟�斤拷1锟�界煡鐢ㄦ埛鐐瑰嚮浜嬩欢澶勭悊
		...
		try {
			...
			case PushType.MUTIPLE_PAY: {
				...
				mNotificationBuilder.setContentTitle(data.getString("sponsor")
						+ "璇锋眰澶氫汉鏀粯");
				mNotificationBuilder.setContentText("鎬诲叡"
						+ data.getInt("number") + "浜哄弬涓庯紝鎮ㄩ渶鍚� "
						+ data.getString("receiver") + " 鏀粯"
						+ data.getDouble("money") + "鍏�");
			...
			case PushType.RECEIVE_MONEY: {
				...
				JSONObject data = message.getJSONObject("data");
				mNotificationBuilder.setTicker("鏀跺埌鏉ヨ嚜 "
						+ data.getString("name") + " 鐨勬眹娆�"
						+ data.getDouble("money") + "鍏�");
				mNotificationBuilder.setContentTitle("鏀跺埌鏉ヨ嚜 "
						+ data.getString("name") + " 鐨勬眹娆�"
						+ data.getDouble("money") + "鍏�");
				mNotificationBuilder.setContentText("鏀跺埌鏉ヨ嚜 "
						+ data.getString("name") + " 鐨勬眹娆�"
						+ data.getDouble("money") + "鍏�");
			}
				break;
	...
	private void paraseContent(final Context context, int errorCode,
		...
		if (errorCode == 0) {
			...
			Log.i("error", "鐧惧害浜戠櫥闄嗘垚鍔燂細" + "appid-->" + appid + "channelid-->"
			...
			if (errorCode == 30607) {
				Log.i("error", "璐﹀彿宸茶繃鏈燂紝璇烽噸鏂扮櫥褰�");
				// 璺宠浆鍒伴噸鏂扮櫥褰曠殑鐣岄潰
			} else {
				Log.i("error", "鍚姩澶辫触锛屾鍦ㄩ噸璇�...");
